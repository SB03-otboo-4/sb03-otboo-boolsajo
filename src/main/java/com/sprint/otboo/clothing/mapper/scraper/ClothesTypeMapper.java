...
public class ClothesTypeMapper {
    ...
    public static ClothesType mapToClothesType(String category) {
        ...
        // ===== 속옷 UNDERWEAR =====
        if (lower.matches(".*(속옷|언더웨어|브라|팬티|란제리|슬립|캐미솔|서포트웨어|underwear|bra|panty|lingerie|slip|camisole|briefs|boxer).*")) {
        ...
        // ===== 양말 SOCKS =====
        if (lower.matches(".*(양말|삭스|스타킹|니삭스|socks|stockings|knee[- ]?high|ankle socks).*")) {
        ...
        // ===== 가방 BAG =====
        if (lower.matches(".*(가방|백|클러치|백팩|토트|크로스백|숄더백|힙색|파우치|캐리어|지갑|서류가방|메신저백|슬링백|버킷백|bag|backpack|clutch|tote|crossbody|shoulder|pouch|hipbag|wallet|suitcase|briefcase|messenger|sling|bucket|fanny pack|duffle).*")) {
